...
describe('Entities json', () => {
    ...
    it("should generate json with FM3 Famix-TypeScript-Entities.Class for EntityClass", () => {
        expect(jsonOutput).toMatch(/"FM3":"Famix-TypeScript-Entities.Class","id":[1-9]\d*|0,"sourceAnchor":{"ref":[1-9]\d*|0},"name":"EntityClass"/);
    });

    it("should contain an EntityClass with three methods", () => {
        const theClass = parsedModel.filter(el => (el.FM3 === "Famix-TypeScript-Entities.Class" && el.name === "EntityClass"))[0];
    ...
    it("should have method move", () => { 
        const theMethod = parsedModel.filter(el => (el.FM3 === "Famix-TypeScript-Entities.Method" && el.name === "move"))[0];
