...
describe('Inheritance', () => {
    ...
    it("should contain a Fish class who has a superclass Animal", () => {
        const fishCls = parsedModel.filter(el => (el.FM3 === "Famix-TypeScript-Entities.Class" && el.name === "Fish"))[0];
    ...
    it("should contain an Animal class who has a subclass Fish", () => {
        const animalCls = parsedModel.filter(el => (el.FM3 === "Famix-TypeScript-Entities.Class" && el.name === "Animal"))[0];
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    it("should contain a Flyable interface", () => {
        const flyableInterface = parsedModel.filter(el => (el.FM3 === "Famix-TypeScript-Entities.Interface" && el.name === "Flyable"))[0];
    ...
    it("should contain a Bird class who has a superclass Animal and implements a Flyable interface", () => {
        ...
        const flyableInterface = parsedModel.filter(el => (el.FM3 === "Famix-TypeScript-Entities.Interface" && el.name === "Flyable"))[0];
        expect(flyableInterface).toBeTruthy();
        const birdCls = parsedModel.filter(el => (el.FM3 === "Famix-TypeScript-Entities.Class" && el.name === "Bird"))[0];
